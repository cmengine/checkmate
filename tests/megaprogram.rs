...
/// `jsonValue!` invocation inside a template-literal island is a REAL
...
"#;
    ...
    // spliced template literal; the raw `name!` text is gone.
    let normalized: String = outcome
        .expanded
        .split_whitespace()
        .collect::<Vec<_>>()
...
/// §8.6 heredoc regions: `name! <<tag … tag` extends the region
...
mega rawEcho(freeform.text as t) {
    $"{$t.t}"
}

int main() {
    str echoed = rawEcho! <<END
...
fn cme_expand_writes_the_sidecar_file() {
    ...
    assert!(!expanded.contains("jsonValue!"));
...
"##
    ...
    assert!(
        !outcome.expanded.contains("grabWords!"),
